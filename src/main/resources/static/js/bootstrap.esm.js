...
class Swipe extends Config {
  ...
  _eventIsPointerPenTouch(event) {
    return this._supportPointerEvents && (event.pointerType === POINTER_TYPE_PEN
...
class Tooltip extends BaseComponent {
  ...
  _createPopper(tip) {
    ...
    return Popper.createPopper(this._element, tip,
  ...
  _configAfterMerge(config) {
    config.container = config.container === false ? document.body : getElement(
        config.container);
  ...
  _activateParents(target) {
    ...
    if (target.classList.contains(CLASS_NAME_DROPDOWN_ITEM)) {
      SelectorEngine.findOne(SELECTOR_DROPDOWN_TOGGLE$1,
          target.closest(SELECTOR_DROPDOWN)).classList.add(CLASS_NAME_ACTIVE$1);
      return;
    }
